...
class MeshbluHttpSpec: QuickSpec {
  override func spec() {
    ...
    class MockHttpRequester : MeshbluHttpRequester {
      var registerResponse : Result<JSON, NSError>!
    ...
    describe(".register") {
      ...
      describe("when successful") {
        beforeEach {
          mockRequester.registerResponse = Result(value: JSON(["uuid":"123"]))
          waitUntil { done in
      ...
      describe("when error") {
        beforeEach {
          let error = NSError()
          mockRequester.registerResponse = Result(error: error)
          waitUntil { done in
            meshblu.register() { (result) in
              responseJSON = result.value
              responseError = result.error
